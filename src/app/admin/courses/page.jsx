...
export default function ManageCoursesPage() {
  ...
  // Update an EXISTING chapter (has a real DB id)
  async function updateChapter(chapterId, title, driveFolderId) {
    ...
    const res = await fetch(`${API_URL}/api/chapters/${chapterId}`, {
      ...
      body: JSON.stringify({ title, drive_folder_id: driveFolderId }),
    });
    if (!res.ok) throw new Error(`Failed to update chapter ${chapterId}`);
    return res.json();
  }

  // Create a NEW chapter linked to a course
  async function createChapter(courseId, title, driveFolderId) {
    await fetch(`${API_URL}/sanctum/csrf-cookie`, { credentials: 'include' });
    const xsrf = getCookie('XSRF-TOKEN');
    const res = await fetch(`${API_URL}/api/courses/${courseId}/chapters`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
  ...
  const emptyForm = () => ({
    ...
    description: '',
    thumbnail_url: '',
    chapters: [{ id: null, title: '', drive_folder_id: '' }],
  });
  ...
  const addChapter = (isEdit = false) => {
    const target = isEdit ? editForm : form;
    const chapters = [...target.chapters, { id: null, title: '', drive_folder_id: '' }];
    ...
    setEditForm({
      ...
      chapters: course.chapters && course.chapters.length > 0
        ? course.chapters.map(ch => ({
            id: ch.id,           // real DB id — will trigger PUT
            title: ch.title || '',
            drive_folder_id: ch.drive_folder_id || '',
          }))
        : [{ id: null, title: '', drive_folder_id: '' }],
    });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // ────────────────────────────────────────────────────────────────────────────
  // Submit: Create or Update Course + Chapter Handling
  // ────────────────────────────────────────────────────────────────────────────
  const handleSubmit = async (e, isEdit = false) => {
    ...
    try {
      ...
      const url = isEdit
        ? `${API_URL}/api/courses/${selectedEditId}`
        : `${API_URL}/api/courses`;
      ...
      // Step 2: Handle chapters
      const courseId = isEdit ? selectedEditId : savedCourse.id;
      const chapters = isEdit ? editForm.chapters : form.chapters;

      await Promise.all(
        chapters
          .filter(ch => ch.title || ch.drive_folder_id) // skip fully empty rows
          .map(ch => {
            if (ch.id === null) {
              // NEW chapter → POST to create it
              return createChapter(courseId, ch.title, ch.drive_folder_id);
            } else {
              // EXISTING chapter → PUT to update it
              return updateChapter(ch.id, ch.title, ch.drive_folder_id);
            }
          })
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => handleChange(e, false)} rows={3}
                  className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  />
                  {form.thumbnail_url && (
                    <button type="button" onClick={() => handleRemoveThumbnail(false)}
                ...
                {form.thumbnail_url && (
                  <img src={form.thumbnail_url} alt="Thumbnail" className="mt-2 w-32 h-32 object-cover rounded" />
                )}
              </div>
              ...
              <div>
                ...
                {form.chapters.map((chapter, index) => (
                  <div key={index} className="border border-gray-200 rounded-md p-4 mb-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                      <input
                        type="text" placeholder="Chapter Title" value={chapter.title}
                        onChange={(e) => handleChapterField(index, 'title', e.target.value, false)}
                        className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      />
                      <input
                        type="text" placeholder="Drive Folder ID" value={chapter.drive_folder_id}
                        onChange={(e) => handleChapterField(index, 'drive_folder_id', e.target.value, false)}
                        className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      />
                    </div>
                    <div className="flex items-center space-x-2">
                      <button
                        type="button" onClick={() => removeChapter(index, false)}
                        className="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600">
                        Remove Chapter
                      </button>
                    </div>
                ...
                <button
                  type="button" onClick={() => addChapter(false)}
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg shadow p-6">
            ...
            <div className="mb-4">
              ...
              <select
                value={selectedEditId} onChange={handleEditSelect}
              ...
              <form onSubmit={(e) => handleSubmit(e, true)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Title</label>
                    <input
                      type="text" name="title" value={editForm.title}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleChange(e, true)} rows={3}
                    className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    {editForm.thumbnail_url && (
                      <button type="button" onClick={() => handleRemoveThumbnail(true)}
                        className="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600">
                        Remove
                      </button>
                    )}
                  ...
                  {editForm.thumbnail_url && (
                    <img src={editForm.thumbnail_url} alt="Thumbnail" className="mt-2 w-32 h-32 object-cover rounded" />
                  )}
                </div>
                ...
                <div>
                  ...
                  {editForm.chapters.map((chapter, index) => (
                    <div key={index} className="border border-gray-200 rounded-md p-4 mb-4">
                      {/* Badge for unsaved new chapters */}
                      {chapter.id === null && (
                        <span className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded mb-2 inline-block">
                          New — will be saved on Update
                        </span>
                      )}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                        <input
                          type="text" placeholder="Chapter Title" value={chapter.title}
                          onChange={(e) => handleChapterField(index, 'title', e.target.value, true)}
                          className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                        />
                        <input
                          type="text" placeholder="Drive Folder ID" value={chapter.drive_folder_id}
                          onChange={(e) => handleChapterField(index, 'drive_folder_id', e.target.value, true)}
                          className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                        />
                      </div>
                      <div className="flex items-center space-x-2">
                        <button
                          type="button" onClick={() => removeChapter(index, true)}
                          className="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600">
                          Remove Chapter
                        </button>
                      </div>
                  ...
                  <button
                    type="button" onClick={() => addChapter(true)}
                    className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">
                    + Add Chapter
                  </button>
                </div>
              ...
              {courses.map((course) => (
                <div key={course.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex space-x-2">
                    <button
                      onClick={() => handleEditSelect({ target: { value: course.id } })}
